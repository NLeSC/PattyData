...
void PotreeWriterNode::loadFromDisk(){
	...
	while(reader->readNextPoint()){
        ...
        Vector3<double> v = Vector3<double>(p.x, p.y, p.z);
		grid->addWithoutCheck(v);
...
PotreeWriterNode *PotreeWriterNode::add(Point &point){
	...
	Vector3<double> v = Vector3<double>(point.x, point.y, point.z);
	bool accepted = grid->add(v);
