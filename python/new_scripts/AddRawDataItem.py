...
def check_required_options(opts,logger):
  logger.info('Checking if all required arguments are specified by the user.')  
  ...
  if ( opts.kind == "SITE" ):
    if not ( opts.siteno ):
      logger.error("[ERROR] site number should be defined for SITE (--siteno)")
  ...
  if ( opts.type == "PC" or opts.type == "MESH" ): 
    if not ( opts.verrecno ):
      logger.error("[ERROR] Version/reconstruction number should be defined (--verrecno)")
      parser.error("Version/reconstruction number should be defined (--verrecno)")
  
def check_directory_structure(RAW_BASEDIR,logger):
  logger.info('Checking if required directory structure exists.')
  # directory structure
  DIRS=[os.path.join(RAW_BASEDIR,PC_DIR,BACKGROUND_DIR), os.path.join(RAW_BASEDIR,PC_DIR,SITES_DIR), \
    os.path.join(RAW_BASEDIR,MESHES_DIR,BACKGROUND_DIR), os.path.join(RAW_BASEDIR,MESHES_DIR,SITES_DIR), \
    os.path.join(RAW_BASEDIR,PICTURES_DIR,BACKGROUND_DIR), os.path.join(RAW_BASEDIR,PICTURES_DIR,SITES_DIR)]
  # check if all directories in the directory structure exist, raise IOError if needed
  for directory in DIRS:
    if not os.path.isdir(directory):
#      os.makedirs(directory) # create directories recursively
      logger.error("[ERROR] Required directory does not exist: " + directory)
...
def define_create_target_dir(opts,logger):
  ...
  inputname = os.path.splitext(os.path.basename(opts.file))[0] # name of input data, only basename, extensions removed
  # TARGETDIR for PC
  if ( opts.type == "PC" and opts.kind == "BACK" ):
    TARGETDIR = os.path.join(target_basedir,inputname+'V'+str(opts.verrecno))
  if ( opts.type == "PC" and opts.kind == "SITE" ):
    if ( opts.aligned ):
      # check if background used for the alignment exists
      if ( os.path.isdir(os.path.join(opts.data,PC_DIR,BACKGROUND_DIR,os.path.splitext(os.path.basename(opts.aligned))[0])) ):
  ...
  if ( opts.type == "MESH" and opts.kind == "BACK" ):
    TARGETDIR = os.path.join(target_basedir,opts.period,inputname+'V'+str(opts.verrecno))
  if ( opts.type == "MESH" and opts.kind == "SITE" ):
    TARGETDIR = os.path.join(target_basedir,'S'+str(opts.siteno),opts.period,inputname+'V'+str(opts.verrecno))
  # TARGETDIR for PICT
  ...
  if ( opts.type == "PICT" and opts.kind == "SITE" ):
    TARGETDIR = os.path.join(target_basedir,'S'+str(opts.siteno),opts.period)
...
def copy_data(opts,TARGETDIR,logger):
  ...
  # if input was a directory: copy everything inside the directory to TARGETDIR
  ...
  # if input was a file: copy the file to TARGETDIR
  elif os.path.isfile(opts.file):
    src_files = os.listdir(opts.file)
    for file_name in src_files:
      if ( os.path.isfile(os.path.join(opts.file, file_name)) ):
    ...
    logger.error("[ERROR] Input file/directory given as argument for --file does not exist: " + opts.file)
    raise IOError("Input file/directory given as argument for --file does not exist: " + opts.file)
  logger.info("Finished copying data to " + TARGETDIR)

def start_logging(filename=LOG_FILENAME, level=DEFAULT_LOG_LEVEL):
  "Start logging with given filename and level."
  logging.basicConfig(filename=filename, level=LOG_LEVELS[level],format=LOG_FORMAT)
...
if __name__ == "__main__":
  ...
  requiredNamedPCMESH = parser.add_argument_group('required arguments for PC and MESH')
  requiredNamedMESHPIC = parser.add_argument_group('required arguments for MESH and PICT') # create a group of required arguments
  requiredNamedPC = parser.add_argument_group('required arguments for PC SITE') # create a group of required arguments
  requiredNamedSITE = parser.add_argument_group('required arguments for SITE')
  # fill argument groups
  parser.add_argument('-i','--data', default=utils.DEFAULT_RAW_DATA_FOLDER, help='RAW data folder [default ' + utils.DEFAULT_RAW_DATA_FOLDER + ']')
  requiredNamed.add_argument('-k','--kind',action='store',help='Type of item',choices=['BACK','SITE'],required=True)
  requiredNamed.add_argument('-t','--type',action='store',help='Type of data',choices=['PC','MESH','PICT'],required=True)
  requiredNamed.add_argument('-f','--file',action='store',help='Input file/directory name to copy',required=True) 
  requiredNamedPCMESH.add_argument('--verrecno',action='store',help='Version or reconstruction number')
  requiredNamedMESHPIC.add_argument('-p','--period',action='store',help='Period (choose from MESH:CURR,ARCH_REC; PICT:CURR,HIST)',choices=['CURR','HIST','ARCH_REC'])
  requiredNamedPC.add_argument('-a','--aligned',action='store',help='Aligned to a specific background') 
  parser.add_argument('--eight',help='8 bit color [only for PC SITE]',action="store_true")
  parser.add_argument('-l','--log',help='Log level',choices=['debug','info','error'],default=DEFAULT_LOG_LEVEL)
  requiredNamedSITE.add_argument('--siteno',action='store',help='Site number')
