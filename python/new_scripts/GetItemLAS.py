...
FROM (
    ...
    queryArgs.extend([args.concave, args.itemid,])
    ...
    listPCFiles = glob.glob(args.las + '/*las') + glob.glob(args.las + '/*laz')
    if len(listPCFiles) == 0:
        logging.error('%s does not contain any LAS/LAZ file' % args.las)
        return
    
    # Create list of files for lasmerge
    tfilename = args.output + '.list' 
    tfile = open(tfilename, 'w')
    for f in listPCFiles:
        tfile.write(f + '\n')
    tfile.close()
    command = 'lasmerge -lof ' + tfilename + ' -inside ' + str(minx) + ' ' + str(miny) + ' ' + str(maxx) + ' ' + str(maxy) + ' -o ' + args.output
    ...
    logging.info('Getting average elevation and number of points from %s' % args.output)
    statcommand = "lasinfo -i " + args.output + " -nv -nmm -histo z 10000000"
