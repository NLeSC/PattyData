...
def createOSG(inFile, outFolder, inType, opts, abOffsetX = None, abOffsetY = None,
              abOffsetZ = None, color8Bit = False):
    ...
    # database connection
    connection, cursor = utils.connectToDB(opts.dbname, opts.dbuser, opts.dbpass,
                                   opts.dbhost, opts.dbport)

    abspath = inFile # is this correct?
    data_items, num_items = utils.fetchDataFromDB(cursor, "SELECT item_id FROM RAW_DATA_ITEM WHERE abs_path = '%s'" % (abspath))
    itemID = data_items[0][0]
        
    # Get 8bitcolor information from DB
    data_items, num_items = utils.fetchDataFromDB(cursor, 'SELECT color_8bit FROM RAW_DATA_ITEM_PC INNER JOIN RAW_DATA_ITEM ON RAW_DATA_ITEM_PC.raw_data_item_id = RAW_DATA_ITEM.raw_data_item_id INNER JOIN ITEM ON RAW_DATA_ITEM.item_id = %s' % (itemID) )
    color8Bit = data_items[0][0] # boolean if 8BC

    # Get alignment info from DB
    data_items, num_items = utils.fetchDataFromDB(cursor, 'SELECT offset_x, offset_y, offset_z FROM OSG_DATA_ITEM_PC_BACKGROUND INNER JOIN RAW_DATA_ITEM ON OSG_DATA_ITEM_PC_BACKGROUND.raw_data_item_id = RAW_DATA_ITEM.raw_data_item_id INNER JOIN ITEM ON RAW_DATA_ITEM.item_id = %s' % (itemID) )
...
#    logger.info(mvcommand)
    ...
    if len(ofiles) == 0:
        logger.error('none OSG file was generated (found in ' + outFolder + '). Check log: ' + logFile)
        mainOsgb = None
    else:
        mainOsgb = ofiles[0]
        if inType != 'bg':
            ...
            if len(xmlfiles) == 0:
                logger.error('none XML file was generated (found in ' + outFolder + '). Check log: ' + logFile)
                xmlPath = None
            else:
                xmlPath = xmlfiles[0]
                if len(xmlfiles) > 1:
                    logger.error('multiple XMLs file were generated (found in ' + outFolder + '). Using ' + xmlPath)
...
def main(opts):
    ...
    logger.info('#######################################')
    createOSG('/home/ronald/pattytest', '/home/ronald/pattytest/outdir', 'PC',opts)
...
if __name__ == "__main__":
    ...
    op.add_option('-l', '--log', type='string', default=utils.DEFAULT_LOG_LEVEL)
