...
def main(opts):
    ...
    if os.popen('cat ' + logFile + ' | grep ERROR').read().count("ERROR"):
        ...
        cursor.execute("INSERT INTO sites_geoms( site_id, geom ) SELECT site_id as site_id, ST_Multi(ST_Transform( ST_Union( geom ), " + str(utils.SRID) + " )) AS geom FROM sites_geom_temp GROUP BY site_id")
